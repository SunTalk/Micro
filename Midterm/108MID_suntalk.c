...
int main()
{
	...
	unsigned int index_7LED[8] = {Digit_8, Digit_7, Digit_6, Digit_5, Digit_4, Digit_3, Digit_2, Digit_1};
	unsigned int index_7LED_NUM[18] = {Number_0, Number_1, Number_2, Number_3, Number_4, Number_5, Number_6, Number_7,
			Number_8, Number_9, Number_A, Number_b, Number_C, Number_d, Number_E, Number_F, Number_Dot,0 };
	...
	int input;
	int pre = -1;
	int led_now,led_state;
	int A,B,Result;

	int LED[8] = {17,17,17,17,17,17,17,17};
	int seg_state;


	while(1){

		input =  (GPIO_PTC_PADIN>>2)&0xFF;
		
		if( pre != input ){
			led_state = 0;
			led_now = (rev(~input)<<8)|rev(input);
			A = (input>>4)&0xF;
			B = input&0xF;
			Result = A*B*abs(A-B);
			seg_state = 0;
		...
		while( led_state < 16 ){
			led_state++;
			led_now = rot(led_now);
			GPIO_PTB_GPIO = led_now;
			delay1(100000);
		}

		for(i=0;i<8;i++) LED[i] = 17;
